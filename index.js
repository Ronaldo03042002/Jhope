...
prefix = "!"
name = "~ Ronaldo | B O T"
rdaftar = "OBRIGADO POR SE REGISTAR, PARA SE TORNAR UM MEMBRO PREMIUM TENTE CONVERSA COM O MEU DONO."
rmenu = "BOT VERIFICADO PELO WHATSAPP ✅"
limitt = 30
ban = []
userpremium = ["557196834782@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiee @${num.split('@')[0]}\nBem vindo(a) ao grupo *${mdata.subject}* Leia as regras para que você não seja expulso do grupo, sinta-se em casa🌚💜`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ Aguarde está em andamento...*',
				success: '*Sucesso...*',
				error: {
				...
				only: {
					...
					benned: '*Desculpe, seu número foi banido, entre em contato com o proprietário para ser desbanido seu ban*',
					ownerG: '*Desculpe, este comando só pode ser usado pelo proprietário!*',
					ownerB: '*Desculpe, este comando só pode ser usado pelo proprietário do bot!* ',
					premium: '*Desculpe, este recurso é apenas para usuários premium!*',
					userB: `Oi ${pushname2} dei uma olhada no meu sistema e notei que você não está registrado no banco de dados. Digite ${prefix}registrar para se registrar no banco de dados do bot!`,
			...
			const ownerNumber = ["557196834782@s.whatsapp.net"] // owner number ubah aja
		...
		case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('*Você já está cadastrado no banco de dados do bot😗*')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = 'Lista de pessoas banidas:\n'
					...
					break 
				case 'premiumlist':
				prem = 'Lista de usuários premium :\n'
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'criador':
                 ...
                 client.sendMessage(from, 'Se você quiser salvar ou bater um papo, fica a vontade🌹',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
				...
				case 'addprem':
					...
					reply(`*Adicionado com sucesso ${addpremium} agora você é um usuário premium *\n\naproveite os recursos premium `)
					break
				case 'banpremium':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'kitar': 
				    if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			    	anu = await client.groupLeave(from, `Adeus pessoal *${groupMetadata.subject}*`, groupId)
	                break
	            case 'getses':
                    if (!isOwner) return client.reply(from, 'Este comando é apenas para proprietários', id)
                ...
                client.sendMessage(from, `\`\`\`✓Alteração da descrição do grupo com sucesso\`\`\` *${groupMetadata.subject}* Torna-se: *${body.slice(9)}*`, text, {quoted: mek})
                break
				case 'tts':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return client.sendMessage(from, 'Qual código de idioma?\n Se você não sabe o código do idioma, basta digitar *${prefix}língua*', text, {quoted: mek})
	            ...
	            if (!isUser) return reply(mess.only.userB)
                    ...
                    reply(mess.wait)
                    reply(`\`\`\`✓Alteração de perfil do grupo com sucesso\`\`\` *${groupMetadata.subject}*`)
                    break
                case 'apakah':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                ...
                client.sendMessage(from, `Rapidez: ${latensi.toFixed(4)} _Segundos_`, text, { quoted: mek})
                    break
                case 'tagme':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						if (isWelkom) return reply('Prontinho✅')
						welkom.push(from)
						...
						reply('on para ativar, off para desabilitar')
					}
					break 
				case 'caklontong':
                if (isBanned) return reply(mess.only.benned)    
				...
				if (!isOwner) return reply(mess.only.ownerB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marque a pessoa que você deseja clonar')
	...
	case 'infocuaca':
     ...
     reply(mess.wait)
         ...
         if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return reply('Que tipo de foto você deseja encontrar?')
					pinte = body.slice(11)
...
// premium user
         case 'play':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *Sua música foi encontrado✅* 」\n\n*• Autoria* : ${anu.result.judul}\n*• Nome da música* : ${anu.result.album}\n*• Publicado em* : ${anu.result.dipublikasi}\n\n*AGUARDE, ESTOU ENVIANDO...*`
       ...
       case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					ytt = `「 *Video encontrado✅* 」\n\n• Título : *${anu.title}*\n• *Tamanho:* ${anu.size}\n• *Duração:* ${anu.duration}\n• *Descrição:* ${anu.desc}\n\n Espere um minuto 1 minuto talvez um pouco mais por causa do download`
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *Sua música foi encontrada* 」\n*• Autoria:* ${anu.result.title}\n*• Fonte:* ${anu.result.source}\n*• Tamanho:* ${anu.result.size}\n\n*AGUARDE ESTOU ENVIADO, POR FAVOR SEM SPAM*`
...
case 'asupan':
		...
		if (isLimit(sender)) return reply(limitend(pushname2))
		 ...
		 case 'tomp3':
				...
				if (!isUser) return reply(mess.only.userB)
                	client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('_*Responda o vídeo!*_')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
				...
				default:
                  ...
                  reply(`*Desculpe, algo deu errado*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[Jhope]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
