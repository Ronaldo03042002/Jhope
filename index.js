...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					...
					Iv: '*Desculpa, seu link é invalido!*'
				},
				only: {
					...
					ownerG: '*Desculpe, este comando só pode ser usado pelo administradores do grupo!*',
		...
		case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Verifiquei o meu sistema e vê que você já está registrado no banco de dados do bot✅')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return client.sendMessage(from, 'Qual código de idioma?*', text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						if (isWelkom) return reply('Você tem sido ativo ou nãon')
						welkom.push(from)
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Desculpa, o comando *${prefix}${command}* não está registrado no banco de dados do bot`)
                  }
