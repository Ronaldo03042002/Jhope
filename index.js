...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Ronaldo\n'
            + 'ORG:Owner FXC7;\n'
            + 'TEL;type=CELL;type=VOICE;waid=557196834782:+55 71 9683-4783\n'
            + 'END:VCARD'
...
limitt = 99999999
ban = []
userpremium = ["557196834782@s.whatsapp.net"]
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oi @${num.split('@')[0]}\nBem vindo ao(a) grupo *${mdata.subject}* dê uma olhadinha nas regras se possivel, sinta-se em casa🌹`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const time = moment.tz('Brasil/Brasília').format('HH:mm:ss')
			const jam = moment.tz('Brasil/Brasília').format('HH:mm')
			...
			mess = {
				...
				error: {
					...
					Iv: '*Desculpa, seu link está inválido!*'
				},
				only: {
					group: '*Desculpe, este comando só pode ser usado em grupos!*',
					benned: '*Desculpe, seu número foi banido, entre em contato com o proprietário para abrir seu ban!*',
					ownerG: '*Desculpe, este comando só pode ser usado pelos administradores do grupo*',
					...
					userB: `Oi ${pushname2}, Você não está registrado no banco de dados. Digite ${prefix}registrar`,
					admin: '*Desculpe, este comando só pode ser usado pelos administradores do grupo!*',
					Badmin: '*Desculpe, este comando só pode ser usado quando o bot se torna administrador!*'
				}
			}
		...
		case 'registrar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```List Banned``` :\n'
					...
					break 
				case 'premium':
				prem = 'ʟɪsᴛᴀ ᴅᴇ ᴜsᴜᴀʀɪᴏs ᴘʀᴇᴍɪᴜᴍ☟\n'
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'criador':
                 ...
                 client.sendMessage(from, 'Esse é o número do meu criador😗',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'toimg':
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (!isQuotedSticker) return reply(' Marque a figurinha que deseja converter!')
	...
	case 'infocuaca':
     ...
     reply(mess.wait)
         ...
         if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return reply('Que tipobde foto deseja encontrar?')
					pinte = body.slice(11)
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`*Desculpa, este comando* *${prefix}${command}* *não está registrado no banco de dados do bot*`)
                  }
